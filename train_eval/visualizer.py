...
class Visualizer:
    ...
    """
    def __init__(self, cfg: Dict, data_root: str, data_dir: str, checkpoint_path: str, example: int, frames: List[int],
        ...
        self.name = name
        self.legend = False
        self.patch_margin = 30
        self.min_diff_patch = 30
    ...
    def visualize_nuscenes(self, output_dir):
        ...
        for n, indices in enumerate(index_list[self.example:self.example+1]):
            if self.frames is not None:
                indices = [indices[i] for i in self.frames]
            ...
            for i,img in enumerate(fancy_img):
                filename = os.path.join(output_dir, 'example' + str(self.example+n) + '_' + self.name + scene  + '_' + str(self.frames[i]) +'.png')
    ...
    def generate_nuscenes_gif(self, idcs: List[int]):
        ...
        for idx in idcs: 
            ...
            if self.example == 5:
                ...
                my_patch = (min_patch[0]-10, min_patch[1]+5, max_patch[0]-10 , max_patch[1]+5)
            ...
            oi = OffsetImage(r_img, zoom=0.012, zorder=500)
            ...
            for n, ann in enumerate(annotations):
                ...
                if ann['instance_token'] == i_t:
                    ...
                    oi = OffsetImage(r_img, zoom=0.012, zorder=5)
                    veh_box = AnnotationBbox(oi, (history[-1, 0], history[-1, 1]), frameon=False)
                    veh_box.zorder = 5
                    ax2.add_artist(veh_box)                 
