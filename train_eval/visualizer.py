...
class Visualizer:
    ...
    def __init__(self, cfg: Dict, data_root: str, data_dir: str, checkpoint_path: str, example: int, 
                    show_preds: bool = True, tf: int = 12, num_modes: int = 10, counterfactual: bool = False, name: str = ''):
        """
    ...
    def visualize_nuscenes(self, output_dir):
        ...
        for n, indices in enumerate(index_list[self.example:self.example+1]):
            ...
            filename = os.path.join(output_dir, 'example_' + str(self.example) + self.name + scene + '30_patch.gif')
            ...
            for i,img in enumerate(fancy_img):
                ...
                filename = os.path.join(output_dir, 'results', 'gifs', 'example' + str(self.example) + scene  + str(i) + '_graph.png')
    ...
    def visualize_graph(self, fig, ax, node_feats, s_next, edge_type, evf_gt, node_seq, fut_xy, pi, cmap_cool, 
                        vehicles_feats,focal_agent, graph):
        """
        ...
        for node_id, node_feat in enumerate(vehicles_feats[0]): 
            if node_feat.sum() == 0:
                break 
            plt.plot(node_feat[-1, 0], node_feat[-1, 1], '*', color='b', markersize=7)
        
        # Insert focal agent in vehicles_feats
        vehicles_feats=np.concatenate((focal_agent, vehicles_feats[0]))
        ...
        plt.plot( focal_agent[0,-1,0], focal_agent[0,-1,1], '*', color='red', markersize=10 ,alpha=1)
    ...
    def generate_nuscenes_gif(self, idcs: List[int]):
        ...
        for idx in idcs[:1]: 
            ...
            my_patch = (min_patch[0] , min_patch[1]+20, max_patch[0] , max_patch[1]+20 )
                ...
                for n, traj in enumerate(predictions['traj'][0]):
                    ...
                    if 'scout' in self.encoder_type and  n==0:
                        self.visualize_graph(fig3,ax3,data['inputs']['map_representation']['lane_node_feats'][0].detach().cpu().numpy(), 
                                        ...
                                        predictions['pi'][0].detach().cpu().numpy(), cmap_cool, data['inputs']['surrounding_agent_representation']['vehicles'].detach().cpu().numpy(), 
                                        data['inputs']['target_agent_representation'].detach().cpu().numpy(), data['inputs']['lanes_graphs'].cpu() )  
